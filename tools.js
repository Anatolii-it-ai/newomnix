...
window.OmnixTools = {
  ...
  ai: {
    title: 'AI-ассистент',
    render(c) {
      c.innerHTML = '<div class="card" style="padding:0; overflow:hidden"><div class="ai-tool-chat-wrap" id="ai-tool-chat"></div></div>';
      const wrap = document.getElementById('ai-tool-chat');
      if (window.omxAI && typeof window.omxAI.renderChat === 'function') {
        window.omxAI.renderChat(wrap);
        if (window.omxAI.hideFab) window.omxAI.hideFab(); // на этой странице чат и так на виду
      } else {
        c.innerHTML = `<div class="card"><p class="muted">${esc(t('ai.err_generic'))}</p></div>`;
      }
    },
  },
